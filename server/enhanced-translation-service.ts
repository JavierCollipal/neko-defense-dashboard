...
const ENHANCED_LANGUAGES: LanguageConfig[] = [
  { code: 'en', name: 'English', nativeName: 'English', flag: '🇬🇧', dir: 'ltr', priority: 1, complexityLevel: 'simple', providers: ['google', 'deepl'] },
  { code: 'es', name: 'Spanish', nativeName: 'Español', flag: '🇪🇸', dir: 'ltr', priority: 2, complexityLevel: 'simple', providers: ['google', 'deepl'] },
  { code: 'zh', name: 'Chinese', nativeName: '中文', flag: '🇨🇳', dir: 'ltr', priority: 3, complexityLevel: 'complex', providers: ['google', 'deepl'] },
  { code: 'hi', name: 'Hindi', nativeName: 'हिन्दी', flag: '🇮🇳', dir: 'ltr', priority: 4, complexityLevel: 'complex', providers: ['google'] },
  { code: 'ar', name: 'Arabic', nativeName: 'العربية', flag: '🇸🇦', dir: 'rtl', priority: 5, complexityLevel: 'complex', providers: ['google'] },
  { code: 'fr', name: 'French', nativeName: 'Français', flag: '🇫🇷', dir: 'ltr', priority: 6, complexityLevel: 'medium', providers: ['google', 'deepl'] },
  { code: 'de', name: 'German', nativeName: 'Deutsch', flag: '🇩🇪', dir: 'ltr', priority: 7, complexityLevel: 'medium', providers: ['google', 'deepl'] },
  { code: 'ja', name: 'Japanese', nativeName: '日本語', flag: '🇯🇵', dir: 'ltr', priority: 8, complexityLevel: 'complex', providers: ['google', 'deepl'] },
  { code: 'ru', name: 'Russian', nativeName: 'Русский', flag: '🇷🇺', dir: 'ltr', priority: 9, complexityLevel: 'medium', providers: ['google', 'deepl'] },
  { code: 'pt', name: 'Portuguese', nativeName: 'Português', flag: '🇧🇷', dir: 'ltr', priority: 10, complexityLevel: 'medium', providers: ['google', 'deepl'] },
  { code: 'ko', name: 'Korean', nativeName: '한국어', flag: '🇰🇷', dir: 'ltr', priority: 11, complexityLevel: 'complex', providers: ['google'] },
  { code: 'it', name: 'Italian', nativeName: 'Italiano', flag: '🇮🇹', dir: 'ltr', priority: 12, complexityLevel: 'medium', providers: ['google', 'deepl'] },
  { code: 'nl', name: 'Dutch', nativeName: 'Nederlands', flag: '🇳🇱', dir: 'ltr', priority: 13, complexityLevel: 'medium', providers: ['google', 'deepl'] },
  { code: 'tr', name: 'Turkish', nativeName: 'Türkçe', flag: '🇹🇷', dir: 'ltr', priority: 14, complexityLevel: 'medium', providers: ['google'] },
  { code: 'pl', name: 'Polish', nativeName: 'Polski', flag: '🇵🇱', dir: 'ltr', priority: 15, complexityLevel: 'medium', providers: ['google', 'deepl'] }
];
...
export class EnhancedTranslationService {
  ...
  async translateText(
    ...
    try {
      ...
      if (!text || typeof text !== 'string' || text.trim().length === 0) {
        return {
          translatedText: text,
          quality: { confidence: 0, fluency: 0, contextRelevance: 0, overallScore: 0 },
          provider: 'none',
          cached: false
        };
      }
      ...
      if (targetLang === sourceLang) {
        return {
          translatedText: text,
          quality: { confidence: 100, fluency: 100, contextRelevance: 100, overallScore: 100 },
          provider: 'none',
          cached: false
        };
      }
      ...
      const targetLanguage = ENHANCED_LANGUAGES.find(l => l.code === targetLang);
      if (!targetLanguage) {
      ...
      const contextHash = context ? this.generateContextHash(text, context) : undefined;

      // Check cache first
      const cached = await this.getCachedTranslation(text, targetLang, sourceLang, contextHash);
      if (cached) {
      ...
      for (const provider of availableProviders) {
        try {
          ...
          if (result) {
            ...
            if (!bestResult || quality.overallScore > bestResult.quality.overallScore) {
              bestResult = {
                ...result,
                quality,
  ...
  async translateDocument(
    ...
    try {
      ...
      if (cachedDoc && this.isCacheValid(cachedDoc)) {
        console.log(`✅ [Translation] Using cached document translation for ${targetLang}`);
        return this.applyDocumentTranslations(document, cachedDoc);
      }

      console.log(`🌐 [Translation] Translating document to ${targetLang} with enhanced service`);
      ...
      for (const field of translatableFields) {
        ...
        if (Array.isArray(value)) {
          translationPromises.push(
            this.translateArray(value, targetLang, sourceLang, context)
              .then(result => ({ field, result }))
          );
        } else if (typeof value === 'string') {
          translationPromises.push(
            this.translateText(value, targetLang, sourceLang, context)
              .then(result => ({ field, result: result.translatedText }))
          );
        } else if (typeof value === 'object') {
          translationPromises.push(
            this.translateObject(value, targetLang, sourceLang, context)
              .then(result => ({ field, result }))
          );
        }
      }
  ...
  private async translateWithProvider(
    ...
    switch (provider.name) {
      ...
      case 'deepl':
        ...
        const fallbackResult = await translate(text, { from: sourceLang, to: targetLang });
  ...
  private async scoreTranslationQuality(
    ...
    for (const [english, translated] of Object.entries(technicalTerms)) {
      if (originalText.includes(english)) {
        if (translatedText.includes(translated) || translatedText.includes(english)) {
          contextRelevance += 5;
        } else {
          contextRelevance -= 10;
        }
  ...
  private calculateQualityMetrics(cache: TranslationCache): TranslationQualityMetrics {
  ...
  private async translateArray(
    ...
    const translations = await Promise.all(
      textArray.map(async (text) => {
        const result = await this.translateText(text, targetLang, sourceLang, context);
  ...
  private async translateObject(
    ...
    for (const [key, value] of Object.entries(obj)) {
      if (typeof value === 'string') {
        const result = await this.translateText(value, targetLang, sourceLang, `${context}.${key}`);
        translated[key] = result.translatedText;
      } else if (Array.isArray(value)) {
        translated[key] = await this.translateArray(value, targetLang, sourceLang, `${context}.${key}`);
      } else {
  ...
  private getTranslatableFields(collectionName: string): string[] {
    const fieldMappings: Record<string, string[]> = {
      'threat_actors': [
        'name', 'categories', 'targets', 'attack_vectors',
        'major_crimes', 'origin', 'position', 'classification'
      ],
      'dina_perpetrators': [
        'fullName', 'position', 'major_crimes', 'significance', 'rank', 'organization'
      ],
      'dina_torture_centers': [
        'name', 'location', 'description', 'known_for'
      ],
      'dina_international_crimes': [
        'crime', 'description', 'location', 'victims'
      ]
    };
  ...
  private async cacheDocumentTranslation(cacheKey: string, translations: any): Promise<void> {
    // Implement document-level caching
  }
  ...
  private async saveDocumentTranslations(
    ...
    try {
      ...
      console.log(`✅ [Translation] Saved ${targetLang} translations to MongoDB`);
    } catch (error) {
  ...
  async getTranslationStats(): Promise<any> {
    try {
      const [cacheStats, usageStats] = await Promise.all([
        this.cacheCollection.aggregate([
          {
            $group: {
              _id: '$targetLang',
              count: { $sum: 1 },
        ...
        this.metricsCollection.aggregate([
          {
            $group: {
              _id: { provider: '$provider', date: { $dateToString: { format: '%Y-%m-%d', date: '$createdAt' } } },
              requests: { $sum: 1 },
              avgQuality: { $avg: '$quality' }
            }
          }
...
export async function enhancedTranslateText(
  ...
  const result = await enhancedService.translateText(text, targetLang, sourceLang, context);
  return result.translatedText;
}
...
export async function enhancedTranslateDocument(
  ...
  return await enhancedService.translateDocument(document, targetLang, collectionName, sourceLang);
}
