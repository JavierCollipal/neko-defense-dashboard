...
async function demonstrateTranslationUX() {
  ...
  page.on('pageerror', (error) => {
    const pageErrorError = {
      message: error.message,
      stack: error.stack,
      timestamp: new Date(),
      url: page.url()
    };
    errors.page.push(pageError);
  ...
  try {
    ...
    if (textarea) {
      await textarea.scrollIntoView({ behavior: 'smooth', block: 'center' });
      await new Promise(resolve => setTimeout(resolve, 2000));
  ...
  try {
    ...
    const performance = {
      ...
      performance_metrics: {
        ...
        console_warnings: errors.console.filter(e => e.type === 'warning').length,
  ...
  console.log(`⚠️ Console warnings: ${errors.console.filter(e => e.type === 'warning').length}`);
