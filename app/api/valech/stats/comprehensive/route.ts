...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cached = await cacheCollection.findOne({}, { sort: { lastCalculated: -1 } });
    ...
    // VICTIM OUTCOMES
    // ============================================================
    const outcomeAggregation = await victimsCollection.aggregate([
      {
        $group: {
          _id: '$outcome',
          count: { $sum: 1 }
    ...
    // ============================================================
    const justiceAggregation = await dinaCollection.aggregate([
      {
        $group: {
    ...
    justiceAggregation.forEach((item: any) => {
      if (item._id === 'CONVICTED-IMPRISONED' || item._id === 'CONVICTED-DECEASED') {
    ...
    const convictionRate = totalPerpetrators > 0
    ...
    // ============================================================
    const qualityAggregation = await victimsCollection.aggregate([
      {
        $group: {
    ...
    // GENDER DISTRIBUTION
    // ============================================================
    const genderAggregation = await victimsCollection.aggregate([
      {
        $group: {
          _id: '$gender',
          count: { $sum: 1 }
    ...
    // TOP DETENTION CENTERS
    // ============================================================
    const topCentersAggregation = await victimsCollection.aggregate([
    ...
    // ============================================================
    const timelineAggregation = await victimsCollection.aggregate([
      { $unwind: '$detentionHistory' },
      {
        $project: {
