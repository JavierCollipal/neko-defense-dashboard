...
export async function GET() {
  ...
  try {
    ...
    const counts = {
      ...
      ransomware: threatActors.filter(
        (actor) =>
          actor.categories?.includes('Ransomware') ||
          actor.categories?.includes('Ransomware-as-a-Service')
      ).length,
      apt: threatActors.filter(
        (actor) =>
          actor.categories?.includes('APT') || actor.threat_level === 'CRITICAL'
      ).length,
      financial: threatActors.filter(
        (actor) =>
          actor.categories?.includes('Financial Crime') ||
          actor.categories?.includes('Cryptocurrency Theft')
      ).length,
      state_sponsored: threatActors.filter(
        (actor) =>
