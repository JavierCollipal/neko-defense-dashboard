...
function ThreatActors() {
  ...
  const fetchThreatActors = async () => {
    ...
    console.log(
      '🔍 [ThreatActors] Fetching threat actors from API, desu~ | Language:',
      userLang
    );
    try {
      setLoading(true);
      ...
      console.log('✅ [ThreatActors] Received data:', {
        count: data.count,
        ...
        <div className="threat-stats">
          ...
          <div className="stat-badge medium">
            <span className="stat-value">
              $
              {threatActors
                .reduce((sum, a) => sum + (a.attribution?.reward_usd || 0), 0)
                .toLocaleString()}
            </span>
      ...
      <div className="threat-actors-grid">
        {filteredActors.map((actor, index) => (
          <div
            ...
            style={{
              borderLeft: `4px solid ${getThreatLevelColor(actor.threat_level)}`,
            }}
          >
            <div className="actor-header">
              ...
              <div className="actor-title">
                ...
                <span className="actor-id">
                  {actor.actor_id || 'unknown_id'}
                </span>
              </div>
              <div
                className="threat-badge"
                style={{
                  backgroundColor: getThreatLevelColor(actor.threat_level),
                }}
              >
            ...
            <div className="actor-info">
              <div className="info-row">
                ...
                <span className="info-value">
                  {actor.origin?.country || 'Unknown'}
                </span>
              </div>
              <div className="info-row">
                <span className="info-label">Status:</span>
                <span
                  className={`status-badge ${(actor.status || 'unknown').toLowerCase()}`}
                >
            ...
            <div className="actor-categories">
              {actor.categories &&
                actor.categories.slice(0, 2).map((cat, i) => (
            ...
            <div className="actor-stats">
              <div className="stat-item">
                ...
                <span className="stat-text">
                  {actor.evidence?.victim_count || 0} victims
                </span>
              </div>
              <div className="stat-item">
                <span className="stat-icon">💸</span>
                <span className="stat-text">
                  $
                  {(
                    (actor.evidence?.financial_damage_usd || 0) / 1000000
                  ).toFixed(0)}
                  M damage
                </span>
              </div>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-header">
              <div className="modal-icon">
                {getTypeIcon(selectedActor.type)}
              </div>
              <div>
                <h2>{selectedActor.name || 'Unknown Actor'}</h2>
                <p className="modal-subtitle">
                  {selectedActor.actor_id || 'unknown_id'}
                </p>
              </div>
              <div
            ...
            <div className="modal-body">
                ...
                <div className="alias-list">
                  {selectedActor.aliases &&
                    selectedActor.aliases.map((alias, i) => (
                      <span key={i} className="alias-tag large">
                        {alias}
                      </span>
                    ))}
                </div>
              ...
              <section className="modal-section">
                ...
                <div className="info-grid">
                  <div className="info-item">
                    ...
                    {selectedActor.origin?.country || 'Unknown'}
                  </div>
                  <div className="info-item">
                    <strong>State Affiliated:</strong>{' '}
                    {selectedActor.origin?.affiliated_state
                      ? '✅ Yes'
                      : '❌ No'}
                  </div>
                  ...
                  {selectedActor.attribution?.reward_usd > 0 && (
                    <div className="info-item highlight">
                      ...
                      {selectedActor.attribution.reward_usd.toLocaleString()}
                    </div>
                  )}
                </div>
              </section>
              ...
              <section className="modal-section">
                ...
                <div className="category-list">
                  {selectedActor.categories &&
                    selectedActor.categories.map((cat, i) => (
                      <span key={i} className="category-tag large">
                        {cat}
                      </span>
                    ))}
                </div>
                  ...
                  {selectedActor.technical_profile?.malware_families && (
                    <div className="tech-item">
                      ...
                      <div className="tech-tags">
                        {selectedActor.technical_profile.malware_families.map(
                          (m, i) => (
                ...
                <div className="evidence-stats">
                  <div className="evidence-stat">
                    <div className="stat-number">
                      {selectedActor.evidence?.victim_count || 0}
                    </div>
                  ...
                  <div className="evidence-stat">
                    <div className="stat-number">
                      $
                      {(
                        (selectedActor.evidence?.financial_damage_usd || 0) /
                        1000000
                      ).toFixed(0)}
                      M
                    </div>
                  ...
                  <div className="evidence-stat">
                    <div className="stat-number">
                      {selectedActor.evidence?.known_incidents?.length || 0}
                    </div>
              ...
              <section className="modal-section">
                ...
                <div className="intelligence-info">
                  <div className="info-item">
                    ...
                    {selectedActor.intelligence?.sources?.join(', ')}
                  </div>
                  <div className="info-item">
                    <strong>Confidence:</strong>
                    <span
                      className={`confidence-badge ${selectedActor.intelligence?.confidence_level?.toLowerCase()}`}
                    >
                  ...
                  <div className="info-item">
                    ...
                    {new Date(
                      selectedActor.intelligence?.last_updated
                    ).toLocaleDateString()}
                  </div>
                </div>
              </section>
