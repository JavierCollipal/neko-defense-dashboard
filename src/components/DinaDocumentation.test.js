...
describe('DinaDocumentation Component', () => {
  ...
  describe('🌐 API Integration Tests', () => {
    it('fetches DINA stats on mount', async () => {
      ...
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith(
          expect.stringContaining('/dina/stats'),
          expect.anything()
        );
      });
    });
    ...
    it('fetches DINA perpetrators on mount', async () => {
      ...
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith(
          expect.stringContaining('/dina/perpetrators'),
          expect.anything()
        );
      });
    });
  ...
  describe('📋 Perpetrators List View Tests', () => {
    ...
    it('shows "no data" message when perpetrators array is empty', async () => {
      ...
      // When API returns empty, component falls back to hardcoded database
      // So we should see hardcoded data, not "no data" message
      await waitFor(() => {
        const perpCards = document.querySelectorAll('.perp-card');
        expect(perpCards.length).toBeGreaterThan(0); // Hardcoded fallback data is shown
      });
    });
